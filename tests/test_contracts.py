...
class VersionParityTests(unittest.TestCase):
    def test_manifest_package_and_python_version_source_are_in_sync(self):
        ...
        self.assertIn('version: "0.1.0-beta.1"', brain)
        self.assertIn('status: public-beta-release-candidate', brain)
        self.assertIn('state_schema_version: "1.0"', brain)
        self.assertIn('__version__ = "0.1.0b1"', version)
        self.assertIn('DISPLAY_VERSION = "0.1.0-beta.1"', version)
        ...
        self.assertIn('ai-verse-brain-vendor-bridge = "aiverse_brain.vendor_bridge:main"', pyproject)
        self.assertIn('from ._version import __version__', module)
        self.assertIn('PACKAGE_VERSION = __version__', installer)

    def test_release_boundaries_and_docs_exist(self):
        readme = (ROOT / "README.md").read_text(encoding="utf-8")
        runtime = (ROOT / "protocol" / "RUNTIME-PIPELINE.md").read_text(encoding="utf-8")
        security = (ROOT / "SECURITY.md").read_text(encoding="utf-8")
        license_text = (ROOT / "LICENSE").read_text(encoding="utf-8")
        release = (ROOT / "docs" / "RELEASE.md").read_text(encoding="utf-8")
        vendors = (ROOT / "docs" / "VENDOR-REASONERS.md").read_text(encoding="utf-8")
        self.assertIn("AI-Verse OS and AI-Verse Memory remain separate repositories", readme)
        self.assertIn("attaches Brain through the local `.aiverse/extensions/registry.json` contract", readme)
        self.assertIn("It does not edit tracked `AI-VERSE.yaml`, `AGENTS.md`, or capability registries", readme)
        self.assertIn("reasoner-only", readme)
        ...
        self.assertIn("Claude Code", vendors)
        self.assertIn("Codex CLI", vendors)
        self.assertIn("Hermes Agent", vendors)
